...
tmp2 <- tmp %>% gather(maxyield, minyield, meanyield, 
                       value = "yvalue", key = "yieldtype")
tmp3 <- tmp2 %>% gather(-yvalue, -yieldtype, 
                        key = "parm", value = "parmvalue")
...
# a bit tricky but nice way to make it easy to plot all parameters against all values
tmp <- cbind.data.frame(yieldsd, parm)
tmp2 <- tmp %>% gather(maxyield, minyield, 
                       meanyield, value = "yvalue", 
                       key = "yieldtype")
tmp3 <- tmp2 %>% gather(-yvalue, -yieldtype, 
                        key = "parm", value = "parmvalue")
ggplot(tmp3, aes(parmvalue, yvalue, col = yieldtype)) +
...
```{r, pccplot}
ggplot(tmpp, aes(parm, PRCC, col = output)) 
+ geom_point(size = 2)
```
