...
estimate = map(f, ~simple_erosion(I = 2, f = .x)) %>% unlist()
...
pnames = c("Timncoeff1", "Timncoeff2", "Pcoeff1", "Pcoeff2", "intercep")
...
parm[, "Tmincoeff1"] <- qnorm(parm_quant[, "Tmincoeff1"], 
                              mean = -0.015, sd = 0.015 / pvar)

# repeat for all parameters
parm[, "Tmincoeff2"] = qnorm(parm_quant[, "Tmincoeff2"], 
                              mean = -0.00846, sd = 0.0846 / pvar)
parm[, "Pcoeff1"] = qnorm(parm_quant[, "Pcoeff1"], 
                              mean = -0.07, sd = 0.07 / pvar)
parm[, "Pcoeff2"] = qnorm(parm_quant[, "Pcoeff2"], 
                              mean = -0.0043, sd = 0.0043 / pvar)
parm[, "intecep"] = qunif(parm_quant[, "intercep"], 
                           min = 0.28 - 0.28 * pvar, 
                           max = 0.28 + 0.28 * pvar)
